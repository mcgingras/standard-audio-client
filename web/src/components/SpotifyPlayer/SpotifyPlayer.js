...
const SpotifyPlayer = ({ uris, setActiveTrack, setCurrentTrackIndex }) => {
   ...
   * pass
   */
  const [isLoading, token] = useSpotify()
    ...
    <div>
      ...
      {!initializing && (
        <div className="flex items-center">
          ...
          <button
            className="border rounded-full border-white p-4 text-white flex"
            onClick={() => {
              togglePlay()
            }}
          >
          ...
          <div className="text-white text-xs mr-1" draggable="false">
            {scrubPb ? msTimeFormat(scrubPb) : msTimeFormat(count.progress)}
          </div>
          <ProgressBar
            value={playback}
...
const ProgressBar = ({ value, setValue, scrubFunction }) => {
  ...
  return (
    <div
      ref={wrapperRef}
      className="progress-slider flex items-center h-4 w-full relative"
    ...
    >
      <div className="progress-slider bg-yellow-700 rounded-sm flex h-1 w-full overflow-hidden">
        <div
          ...
          style={{
            transform: `translate(-${((1 - (scrub || value)) * 100).toFixed(
              2
            )}%)`,
          }}
        ></div>
      ...
      <button
        className="progress-slider bg-white rounded-lg w-3 h-3 z-5 shadow-md absolute left-0"
